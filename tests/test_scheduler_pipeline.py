...
def test_encode_worker_requests_process_token_when_cancelled(
    ...
    async def scenario() -> None:
        task = asyncio.create_task(execute_encode_job(job_id=1, runner_id="runner", config=config))
...
def test_encode_worker_uses_plan_parallelism_without_resource_selection(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    config, _plan = _stored_encode_job(tmp_path)
    ...
    def fake_execute_plan(
        ...
        *,
        ...
        del cancellation_token, progress_sink
        assert selected_plan.av1an.workers == 2
        return "completed"
    ...
    assert set(command) == {"av1an_argv", "mux_spec"}
    argv = command["av1an_argv"]
