"""add performance environment signature

Revision ID: 0011_performance_environment_signature
Revises: 0010_performance_observations
...
revision: str = "0011_performance_environment_signature"
down_revision: str | None = "0010_performance_observations"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None

_COLUMNS = (
    ("environment_signature_hash", sqlmodel.sql.sqltypes.AutoString()),
    ("environment_signature_json", sqlmodel.sql.sqltypes.AutoString()),
)
...
def upgrade() -> None:
    ...
    _create_index_if_missing(
        ...
        name="ix_performance_observation_environment_signature_hash",
        columns=["environment_signature_hash"],
    )
...
def downgrade() -> None:
    inspector = sa.inspect(op.get_bind())
    existing = {column["name"] for column in inspector.get_columns("performance_observation")}
    if "ix_performance_observation_environment_signature_hash" in {
        index["name"] for index in inspector.get_indexes("performance_observation")
    }:
        op.drop_index(
            "ix_performance_observation_environment_signature_hash",
