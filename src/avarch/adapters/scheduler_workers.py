...
from avarch.adapters.execution import build_av1an_command, execute_plan
...
async def execute_encode_job(
    *,
    ...
    with Session(engine) as session, session.begin():
        job = require_job(session, job_id)
        scene_completed = scene_detection_completed(session, job=job)
    ...
    with Session(engine) as session, session.begin():
        ...
        attempt.command_json = canonical_json(
            {
                "av1an_argv": build_av1an_command(plan.av1an),
                "mux_spec": {
    ...
    execution_progress_sink: ProgressSink = (
        _ProgressFanoutSink((persisted_progress_sink, progress_sink))
    ...
    *,
    ...
    with Session(engine) as session, session.begin():
        ...
        record_stage_event(
            ...
            job_id=job_id,
            attempt=attempt,
            event_type=JobEventType.STAGE_COMPLETED if report.passed else JobEventType.STAGE_FAILED,
